...
function mapBoolean (quad) {
  if (!ns.xsd.boolean.equals(quad.object.datatype)) {
    return quad
  }

  if (quad.object.value === 'Ja') {
    return rdf.quad(quad.subject, quad.predicate, rdf.literal('true', ns.xsd.boolean), quad.graph)
  }

  if (quad.object.value === 'Nein') {
    return rdf.quad(quad.subject, quad.predicate, rdf.literal('false', ns.xsd.boolean), quad.graph)
  }
...
function mapLiteral (quad) {
  return rdf.quad(quad.subject, quad.predicate, rdf.literal(decodeURIComponent(quad.object.value)), quad.graph)
}
...
function mapHomepage (quad) {
  ...
  let value = quad.object.value.match(/null(.*)/)[1]
...
function mapBaumschutzstatus (quad) {
  if (quad.object.termType !== 'Literal') {
    return quad
  }

  const decodeTable = {
    'Gemäss Baumschutzgesetz (BSchG)': '1',
    'Geschützt (Umfang)': '2',
    'Geschützt (Ersatzpflanzung)': '3',
    'Nicht geschützt (BL, Riehen, Waldzone, etc.)': '4'
  }

  const decodedValue = decodeTable[quad.object.value]

  if (!decodedValue) {
    throw new Error(`can't map baumschutzstatus value ${value}`)
  }

  return rdf.quad(quad.subject, quad.predicate, ns.bscode(`baumschutzstatus/${decodedValue}`), quad.graph)
}

const mapByFunctions = new TermMap([
  [ns.schema.addressLocality, mapContainedInPlace],
  [ns.bsproperty.baumschutzstatus, mapBaumschutzstatus],
  [ns.bsproperty.bestehend, mapBoolean],
  ...
  [ns.schema.streetAddress, mapLiteral],
