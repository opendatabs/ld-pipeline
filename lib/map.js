...
function mapDateTime (quad) {
  ...
  if ((/\d\d.\d\d.\d\d\d\d \d\d:\d\d/).test(quad.object.value)) {
    const value = DateTime.fromFormat(quad.object.value, 'dd.MM.yyyy HH:mm', { zone: 'UTC' })
  ...
  if ((/\d\d.\d\d.\d\d\d\d \d\d:\d\d:\d\d/).test(quad.object.value)) {
    const value = DateTime.fromFormat(quad.object.value, 'dd.MM.yyyy HH:mm:ss', { zone: 'UTC' })
