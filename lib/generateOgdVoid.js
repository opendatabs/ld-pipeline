...
class GenerateOgdVoid extends Readable {
  ...
  async _read () {
    ...
    for (const cube of cubes.toArray()) {
      ...
      const id = (cube.value.match(/100[0-9]+$/) || [])[0] // match only dataset id of data.bs.ch
  ...
  async readStaticVoid () {
    ...
    const quadStream = rdf.fromFile('./input/static/void-static.ttl')
  ...
  createDataset ({ json }) {
    ...
    ptr
      ...
      .addOut(ns.dcterms.title, rdf.literal(json.metas.title, 'de'))
      .addOut(ns.dcterms.publisher, ns.bs('org/fachstelle-ogd'))
      .addOut(ns.dcterms.creator, rdf.literal(json.interop_metas.dcat.creator, 'de'))
      .addOut(ns.dcterms.contributor, ns.bs('org/zazuko'))
      .addOut(ns.dcterms.description, rdf.literal(json.metas.description, 'de'))
      .addOut(ns.dcterms.created, rdf.literal(json.interop_metas.dcat.created, ns.xsd.date))
      .addOut(ns.dcterms.modified, rdf.literal(json.metas.modified, ns.xsd.date))

    for (const keyword of json.metas.keyword) {
      ptr.addOut(ns.dcterms.subject, rdf.literal(keyword, 'de'))
    }
